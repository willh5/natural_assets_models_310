...
for i in range(n_patches):
    with rio.open('/Users/william/PycharmProjects/TCN/wlpatches/patch_%s.tif'%i) as data:
...
train_masks=[]
for i in range(n_patches):
    with rio.open('/Users/william/PycharmProjects/TCN/wlmasks/mask_%s.tif'%i) as data:
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0005), loss=total_loss, metrics=metrics)
...
m1=model.fit(X_train, y_train, batch_size=8,verbose=1, epochs=75, validation_data=(X_test,y_test), shuffle=True)
...
weightsave='test_6class_highdropout_nonemiddle_ce.hdf5'
